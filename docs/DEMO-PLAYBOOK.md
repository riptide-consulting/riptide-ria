...
## Tuning the demo to the audience

**Pick the agencies for the prospect.** `config/pipeline_config.json`'s
`federal_register.agencies` currently monitors CMS + FDA. For a payer or health-services
audience, CMS alone is the sharper demo (their world); for device or pharma, FDA. A live
document from the agency they answer to lands harder than any slide.

**Pick the moment for the seat.**
- Compliance or operations leadership: the star is the finished briefing. Open the DOCX,
  walk the remediation table with real due dates, then show the gate refusing the Notion
  write without approval. Their takeaway: hours become minutes, and nothing acts without
  sign-off.
- Security, architecture, or AI-governance reviewers: the star is the governance. Show the
  gate refusal first, then run the injection eval live (`pytest
  evaluations/test_injection_evals.py -q`) and let them watch injected directives fail to
  move the system, then open `logs/audit.jsonl`. Hand them `docs/ENTERPRISE-FAQ.md` on the
  way out; it is written for them.
