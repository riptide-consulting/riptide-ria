...
1. **Python 3.11+**, then from the repo root:
   ...
   ```
2. **Copy `.env.example` to `.env`**, fill in `ANTHROPIC_API_KEY`. That is the only
   required credential: ingest, classify, analyze, and evaluate all run with just it.
   `NOTION_API_KEY` / `NOTION_DATABASE_ID` make the Evaluator's precedent lookups return
   real results and are required for `--synthesize`'s gated Notion write. Google
   credentials (Drive/Gmail) are optional; the pipeline runs fine without them,
   specialists just honestly report Drive wasn't checked.
...
One platform note: `.mcp.json` points Claude Code at the venv's Python using the Windows
path (`.venv\Scripts\python.exe`), because Claude Code launches MCP servers without an
activated venv. On macOS/Linux, change that command to `.venv/bin/python`. The pipeline
itself (`main.py`, `run_demo.py`) is already cross-platform.

## 1. Before every run: a 2-minute pre-flight check

1. **Credentials present?** `.env` needs `ANTHROPIC_API_KEY`; add the Notion pair for a
   demo that shows precedent lookups or the gated write. Confirm with:
   ```
...
| Branded output templates | `config/riptide_template.docx` / `.pptx` | Structural shells today (standard layouts, no branding); drop real branded files in at the same paths, keeping title/content/title-only layouts at positions 0/1/5, and the Synthesizer picks them up unchanged. A white-labeled deliverable for a specific client would need separate template files and a way to select between them, not built yet, since this demo only needs Riptide's own brand. |
