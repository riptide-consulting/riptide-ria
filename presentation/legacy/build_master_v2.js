...
function slide(kicker, title, opts = {}) {
  ...
  const s = p.addSlide();
  s.background = { color: dark ? ABYSS : BONE };
  s.addText(kicker.toUpperCase(), { x: M, y: 0.44, w: 7.2, h: 0.24, fontFace: MONO,
    fontSize: 8, bold: true, charSpacing: 2, color: TEAL, margin: 0 });
  ...
  s.addText(title, { x: M, y: 0.72, w: opts.titleW || CW, h: 0.92, fontFace: HEAD,
    fontSize: opts.titleSize || 22, bold: true, color: dark ? WHITE : ABYSS,
    margin: 0, lineSpacing: 29, valign: "top" });
  s.addShape("line", { x: M, y: 1.76, w: CW, h: 0,
...
const panel = (s, x, y, w, h, fill, border) => s.addShape("rect", { x, y, w, h,
  fill: { color: fill }, line: { color: border || MIST, width: 0.75 } });

/* ---- code block: bone field, teal left rule, mono ---- */
function code(s, x, y, w, h, text, fs) {
  s.addShape("rect", { x, y, w, h, fill: { color: WHITE }, line: { color: MIST, width: 0.75 } });
  s.addShape("rect", { x, y, w: 0.035, h, fill: { color: TEAL }, line: { color: TEAL, width: 0 } });
...
function mark(s, n, x, y, fill, txt) {
  s.addShape("rect", { x, y, w: 0.34, h: 0.34, fill: { color: fill }, line: { color: fill, width: 0 } });
  s.addText(String(n), { x, y, w: 0.34, h: 0.34, align: "center", valign: "middle",
    fontFace: MONO, fontSize: 10, bold: true, color: txt, margin: 0 });
}
...
const parts = [
  ...
  ["02", "The business case", "9-18", "Outcomes, unit economics, operations, legal, security, risk, pilot", "Business, legal, operations", "12 min"],
  ["03", "Technical architecture", "19-32", "Six agents, the code, dependencies, reliability, evals", "Engineering, security", "15 min"],
];
parts.forEach((r, i) => {
  const y = CT + 0.12 + i * 1.42;
  panel(s, M, y, CW, 1.18, i === 1 ? ABYSS : WHITE, i === 1 ? ABYSS : MIST);
  const dk = i === 1;
  s.addText(r[0], { x: M + 0.3, y: y + 0.18, w: 0.8, h: 0.5, fontFace: SERIF, fontSize: 26,
    bold: true, color: dk ? TEAL : MIST, margin: 0 });
  s.addText(r[1], { x: M + 1.15, y: y + 0.24, w: 3.0, h: 0.4, fontFace: HEAD, fontSize: 15,
    bold: true, color: dk ? WHITE : ABYSS, margin: 0 });
  s.addText("Slides " + r[2], { x: M + 1.15, y: y + 0.66, w: 3.0, h: 0.3, fontFace: MONO,
    fontSize: 8.5, color: dk ? DKTEXT : SLATE, margin: 0 });
  s.addText(r[3], { x: M + 4.35, y: y + 0.3, w: 4.6, h: 0.7, fontFace: BODY, fontSize: 10.5,
    color: dk ? DKTEXT : SLATE, margin: 0, lineSpacing: 14 });
  s.addText(r[4], { x: M + 9.15, y: y + 0.24, w: 2.4, h: 0.4, align: "right", fontFace: BODY,
    fontSize: 10, color: dk ? DKTEXT : SLATE, margin: 0 });
  s.addText(r[5], { x: M + 9.15, y: y + 0.62, w: 2.4, h: 0.35, align: "right", fontFace: HEAD,
    fontSize: 12, bold: true, color: dk ? TEAL : ABYSS, margin: 0 });
});
...
kpis.forEach((k, i) => {
  ...
  s.addText(k[1], { x: x + 0.3, y: CT + 1.28, w: 3.3, h: 0.6, fontFace: BODY, fontSize: 11,
...
/* ============================================================
   6 · TRUST MODEL (dark)
   ============================================================ */
s = slide("The trust model", "Models write opinions. Code applies the rules.\nA human holds the key.",
  { dark: true, tracker: "Part 1 · The system", titleSize: 26 });
const cols = [
...
cols.forEach((c, i) => {
  const x = M + i * 4.02;
  s.addShape("rect", { x, y: CT + 0.35, w: 3.66, h: 0.035, fill: { color: i === 2 ? AMBER : TEAL },
    line: { color: i === 2 ? AMBER : TEAL, width: 0 } });
  s.addText(c[0], { x, y: CT + 0.55, w: 3.6, h: 0.4, fontFace: HEAD, fontSize: 13, bold: true,
    charSpacing: 1.5, color: WHITE, margin: 0 });
  s.addText(c[1], { x, y: CT + 1.05, w: 3.6, h: 2.6, fontFace: BODY, fontSize: 11.5,
...
tiers.forEach((t, i) => {
  ...
  s.addShape("rect", { x, y: CT + 0.15, w: 3.66, h: 0.05, fill: { color: t[3] }, line: { color: t[3], width: 0 } });
  s.addText(t[0], { x: x + 0.3, y: CT + 0.45, w: 1.0, h: 1.0, fontFace: SERIF, fontSize: 46,
    bold: true, color: t[3], margin: 0 });
...
grid(s, 6.6, CT + 0.75, 5.73, [
  ...
  cellColor: (i, j, v) => (j === 3 && v === "High") ? AMBER : ABYSS });
...
wins.forEach((w, i) => {
  ...
  s.addText(w[2], { x: M + 9.65, y: y + 0.16, w: 2.3, h: 0.4, align: "right", fontFace: MONO,
    fontSize: 8, charSpacing: 0.5, color: TEAL, margin: 0 });
});
...
const ev = [
  ...
  ["Attacked", "adversarial injection evals try to steer it with hostile document text", AMBER],
...
s.addText(String(PG), { x: 12.0, y: SRC, w: 0.63, h: 0.28, align: "right", fontFace: MONO,
  fontSize: 8.5, color: DKRULE, margin: 0 });
s.addNotes("Transition: everything claimed in parts 1 and 2 maps to a file in a public repository, and this part walks it. Invite the engineers to open the repo alongside; every slide names its files.");
...
s = slide("System architecture", "One document's path: judgment, then code, then a human",
  { tracker: "Part 3 · Technical architecture" });
...
grid(s, M, CT + 0.1, CW, [
  ...
  cellColor: (i, j) => (i === 5 && (j === 3 || j === 4)) ? AMBER : (j === 1 ? SLATE : ABYSS) });
...
s = slide("Agent 5 · Evaluator", "The expensive model grades the work; it cannot act on it",
  { dark: true, tracker: "Part 3 · Technical architecture" });
s.addText([
...
/* ============================================================
   25 · TIER MATRIX
   ============================================================ */
s = slide("Autonomy tiers", "The tier decision, as a lookup table", { tracker: "Part 3 · Technical architecture" });
const mrows = [
...
grid(s, M, CT + 0.2, CW, mrows, [2.73, 1.84, 1.84, 1.84, 1.84, 1.84], {
  ...
  cellColor: (i, j, v) => v.startsWith("1") ? TEAL : v.startsWith("3") ? AMBER : SLATE,
});
...
s.addText("enforcement_detected = true overrides every cell to tier 3.", { x: M + 0.24, y: CT + 2.78,
  w: 8, h: 0.35, fontFace: HEAD, fontSize: 12, bold: true, color: AMBER, margin: 0 });
s.addText([
...
s.addText("THE TWO GATED WRITES", { x: 7.6, y: CT + 1.2, w: 4.7, h: 0.3, fontFace: MONO, fontSize: 8.5,
  bold: true, charSpacing: 1.2, color: AMBER, margin: 0 });
s.addText([
...
s.addText("CREDENTIALS", { x: 6.83, y: CT + 0.1, w: 5, h: 0.28, fontFace: MONO, fontSize: 8,
  bold: true, charSpacing: 1.2, color: SLATE, margin: 0 });
const creds = [
  ["ANTHROPIC_API_KEY", "Required", "Ingest, classify, analyze, evaluate all run on this alone", TEAL],
  ...
  ["RIA_EVALUATOR_APPROVED", "Human key", "Not a credential: the explicit per-run approval for external writes", AMBER],
];
creds.forEach((c, i) => {
...
/* ============================================================
   31 · EVALS
   ============================================================ */
s = slide("Evals", "Behavior is measured, including under attack", { tracker: "Part 3 · Technical architecture" });
s.addText([
...
/* ============================================================
   32 · READING ORDER
   ============================================================ */
s = slide("Reading order", "Where everything lives", { tracker: "Part 3 · Technical architecture" });
...
p.writeFile({ fileName: "/home/claude/Riptide-RIA-Master-Deck.pptx" })
