...
const BG_DARK  = { path: ASSET("bg_abyss.png") };
const BG_LIGHT = { path: ASSET("bg_bone.png") };
...
function slide(kicker, title, opts = {}) {
  ...
  if (opts.tracker) s.addText(opts.tracker.toUpperCase(), { x: 6.5, y: 0.44, w: 6.13, h: 0.24,
    ...
    color: dark ? DKTEXT : SLATE, margin: 0 });
  ...
  if (!opts.noNum) s.addText(String(PG), { x: 12.0, y: SRC, w: 0.63, h: 0.28, align: "right",
    fontFace: MONO, fontSize: 8.5, color: dark ? DKTEXT : SLATE, margin: 0 });
  return s;
}
const source = (s, txt, dark) => s.addText(txt, { x: M, y: SRC, w: 10.9, h: 0.28,
  fontFace: BODY, fontSize: 8, color: dark ? DKTEXT : SLATE, margin: 0 });
...
s.addText("Federal Register publication to an owner-assigned remediation plan in about five minutes,\nunder a trust model where models write opinions, code applies the rules, and a human holds the key. Companion: Riptide Attest -- the proof to this system's opinion.",
...
s.addText("Anthropic-first AI strategy and engineering   ·   Claude Partner Network   ·   Carlsbad, CA",
  { x: 0.75, y: 6.8, w: 11, h: 0.3, fontFace: BODY, fontSize: 9, color: DKTEXT, margin: 0 });
...
const parts = [
  ["01", "The system", "3-9", "What it is, how it decides, what it delivers", "Every audience", "10 min"],
  ["02", "The business case", "10-19", "Outcomes, unit economics, operations, legal, risk, pilot", "Business, legal, operations", "12 min"],
  ["03", "The logical architecture", "20-24", "Capabilities, contracts, invariants, ports. No product names.", "Architecture review", "8 min"],
  ["04", "Technical architecture", "25-38", "Six agents, the code, dependencies, reliability, evals", "Engineering, security", "15 min"],
];
parts.forEach((r, i) => {
  ...
  s.addText(r[0], { x: M + 0.28, y: y + 0.14, w: 0.8, h: 0.45, fontFace: SERIF, fontSize: 23,
    bold: true, color: dk ? TEAL : SLATE, margin: 0 });
...
s = slide("What RIA does", "Publication in, owner-assigned remediation plan out, in about five minutes",
  { tracker: "Part 1 · The system" });
const kpis = [["~5 min", "publication to finished briefing", ABYSS, ABYSS],
               ["$0.59", "per document, measured live", ABYSS, ABYSS],
               ["100%", "of external actions human-gated", DTEAL, TEAL]];
kpis.forEach((k, i) => {
  const x = M + i * 4.02;
  s.addShape("rect", { x, y: CT + 0.1, w: 0.035, h: 1.9, fill: { color: k[3] }, line: { color: k[3], width: 0 } });
...
grid(s, 6.6, CT + 0.75, 5.73, [
  ...
  ["Enforce 15-min portal session timeout  ·  ATTEST", "Security Engineering", "Aug 01", "High"],
  ["Set audit log retention at 180 days  ·  ATTEST", "Security Engineering", "Aug 15", "High"],
  ["Train teams on consent workflow  ·  HUMAN", "Compliance PMO", "Sep 01", "Medium"],
], [2.35, 1.63, 0.85, 0.9], { fs: 9.5, rowH: 0.6,
  cellColor: (i, j, v) => (j === 3 && v === "High") ? DAMBER : ABYSS });
source(s, "Briefing 2026-14012 -- the worked example carried through the Attest deck and both repositories. ATTEST-tagged actions compile into approved specs; closure becomes a continuously re-verified verdict.");
...
/* ============================================================
   9 · THE PAIRED SYSTEM (dark)
   ============================================================ */
s = slide("The paired system", "RIA writes the opinion.\nAttest carries the proof.\nTogether: one governed loop.",
  { dark: true, tracker: "Part 1 · The system", titleSize: 30, titleH: 1.85, ruleY: 2.62 });
const pair = [
  ["RIPTIDE RIA", "The judgment half. Models read each publication and write opinions -- priority, materiality, gaps, a remediation plan. Code computes the autonomy tier; a human key gates every external write. Judgment recurs per document, because judgment is the product."],
  ["RIPTIDE ATTEST", "The verification half. A model compiles each machine-checkable action into a formal spec exactly once; a named human pins its SHA-256; a pure engine then proves the control -- continuously, reproducibly, byte for byte, at zero marginal model cost."],
  ["THE HANDOFF", "The remediation plan is the contract. Attest's triage reads this briefing's plan, compiles the checkable actions, and routes the rest to the human tracker. Briefing 2026-14012 is carried through both decks and both repositories, end to end."],
];
pair.forEach((c, i) => {
  const x = M + i * 4.02;
  s.addShape("rect", { x, y: CT + 0.95, w: 3.66, h: 0.045, fill: { color: i === 2 ? AMBER : TEAL },
...
source(s, "Companion deck: Riptide-Attest-Master-Deck. Companion repository: github.com/riptide-consulting/riptide-attest.", true);
s.addNotes("The handoff in one sentence: RIA's remediation plan is the contract, and Attest compiles its checkable actions into human-approved specs whose closure is re-verified continuously while the rest routes to people. Briefing 2026-14012 is the same worked example on both sides of the pair.");
...
s.addText(String(PG), { x: 12.0, y: SRC, w: 0.63, h: 0.28, align: "right", fontFace: MONO,
  fontSize: 8.5, color: DKTEXT, margin: 0 });
...
source(s, "The 30-day pilot replaces the right-hand variables with your measured numbers. Second-order term, unpriced here: avoided cost of a missed change. Third: each plan's closure is verified downstream by Riptide Attest at zero marginal model spend -- the loop closes without recurring cost.");
...
s.addText([
  ...
  { text: "Headcount unchanged: capacity moves to review and remediation; closure is verified by Attest attestations, not follow-up meetings", options: { bullet: true } },
...
const risks = [
  ...
  ["Residual risk, stated", "Prompt framing is evidence, not proof, against attacks nobody has written yet. Keyword backstops are heuristics. Both are watched by evals rather than assumed away, and both are survivable because the tier and the gates are code. One risk retired since first writing: plan execution, previously unverifiable, is now bounded by the companion verifier, Riptide Attest.", true],
];
risks.forEach((r, i) => {
...
source(s, "Everything on the left is live and measured, not roadmap. Optional extension: compile the pilot's remediation plan into Riptide Attest and close the loop -- publication to plan to attestation.");
...
s.addImage({ path: ASSET("ria_logical.png"), x: M, y: CT + 0.05, w: 8.4, h: 4.725 });
const lt = [
...
grid(s, M, CT + 0.05, CW, [
  ...
  ["Briefing", "Synthesis", "summary, risk assessment, remediation actions", "Artifacts, Gated Execution, Riptide Attest (Control Source Port)"],
...
panel(s, M, CT + 4.22, CW, 0.46, ABYSS, ABYSS);
s.addText("Plus two operating invariants: every decision is recorded before it is trusted, and consumption is bounded by a ceiling.",
  { x: M + 0.3, y: CT + 4.22, w: 11.3, h: 0.46, fontFace: BODY, fontSize: 10.5, color: WHITE, valign: "middle", margin: 0 });
...
grid(s, M, CT + 0.05, CW, [
  ...
  ["Work Item Port", "Notion record", "ServiceNow or Jira -- or Riptide Attest (verification)"],
...
s.addImage({ path: ASSET("ria_arch_brand.png"), x: M, y: CT + 0.05, w: 8.4, h: 4.725 });
const takeaways = [
...
s.addText("The same trust model that governs the pipeline governs its own development: models propose, deterministic gates dispose, a human approves the merge.",
  { x: M, y: CT + 4.15, w: 11.5, h: 0.65, fontFace: SERIF, fontSize: 12.5, italic: true, color: ABYSS, margin: 0 });
...
s.addText("Watch it refuse.", { x: 0.75, y: 2.0, w: 11.9, h: 1.2, fontFace: HEAD, fontSize: 50,
  bold: true, color: WHITE, margin: 0 });
s.addText("The live demo runs a real document published this week, end to end, and finishes with the system declining to touch your tracker or your inbox because no human turned the key. That refusal is the product. Then its companion proves the fix: RIA writes the opinion; Attest carries the proof.",
  { x: 0.75, y: 3.35, w: 9.8, h: 1.45, fontFace: SERIF, fontSize: 16, color: TEAL, margin: 0, lineSpacing: 25 });
s.addText("github.com/riptide-consulting/riptide-ria   ·   companion: github.com/riptide-consulting/riptide-attest   ·   docs/ENTERPRISE-FAQ.md",
...
s.addText("Anthropic-first AI strategy and engineering  ·  Claude Partner Network  ·  Claude Certified Architect",
  { x: 0.75, y: 6.5, w: 11, h: 0.3, fontFace: BODY, fontSize: 9, color: DKTEXT, margin: 0 });
s.addText(String(PG), { x: 12.0, y: SRC, w: 0.63, h: 0.28, align: "right", fontFace: MONO,
  fontSize: 8.5, color: DKTEXT, margin: 0 });
...
p.writeFile({ fileName: _np.join(OUT, "Riptide-RIA-Master-Deck.pptx") })
