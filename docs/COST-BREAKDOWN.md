...
| **Evaluator** | Opus 4.8 | **The one deliberately expensive stage.** This is the trust boundary — the single decision that determines whether anything auto-executes. It's also genuinely agentic: it decides for itself whether and how many times to call a live precedent-lookup tool before answering, which is real reasoning work a cheaper model does less reliably. Every other stage uses a cheaper tier precisely so the budget can concentrate here, where judgment quality has the highest consequence. | **$0.33 – $0.50** (real range, 4 live runs) | **~170-215 sec** |
...
Call it **~$0.59 per document**, ingest through a finished, branded DOCX and PPTX. Wall clock:
