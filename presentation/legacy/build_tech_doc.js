...
const NAVY="0B2545", MID="13416B", TEAL="2E8BA6", INK="1B2B3A", MUTE="5A7184",
      WARN="C94F4F", CODEBG="EDF3F7", CARD="F4F8FA";
const H_F="Century Gothic", B_F="Trebuchet MS", M_F="Consolas";
...
children.push(new Paragraph({ spacing:{ after: 120 },
  children:[ new TextRun({ text:"Technical Documentation", font:H_F, size:40, bold:true, color:TEAL }) ] }));
...
["1. System overview","2. Launch guide","3. The agents, one by one","4. The trust gate, in code",
 "5. Shared infrastructure","6. Why the decisions were made","7. Data flows and security summary",
 "8. Appendix: dependencies and repository map"].forEach(t => children.push(body(t, { after: 60 })));
...
children.push(new Paragraph({ spacing:{ after: 80 }, alignment: AlignmentType.CENTER,
  children:[ new ImageRun({ type:"png", data: fs.readFileSync("/home/claude/ria_architecture.png"),
...
children.push(body("Five stages run under main.py's orchestration: ingest, classify, analyze, evaluate, synthesize. Each stage is isolated per document (one failure logs and the run continues), cost accrues against a hard spend ceiling between documents, and every decision writes one structured log line."));
...
children.push(bullet([{text:"Expected gating message, not an error: ", bold:true},{text:"without RIA_EVALUATOR_APPROVED set, --synthesize logs that the Notion write and escalation email were skipped. That is the specified behavior of the gates in section 4."}]));
...
children.push(step("steps5","Review per-call costs in logs/ria.log to confirm the spend matches expectation (evaluate dominates; section 3, table 1)."));
...
// ---------- 3. agents ----------
children.push(h1("3. The agents, one by one"));
...
children.push(caption("Table 1. The roster. Costs and latencies measured from live runs (docs/COST-BREAKDOWN.md). Latency is API-side inference time; host hardware is not a factor (section 2.4)."));

children.push(h2("3.1 Classifier (ria/classifier.py)"));
...
children.push(h2("3.2 The specialists (ria/specialists.py)"));
...
children.push(h2("3.3 Evaluator (ria/evaluator.py)"));
...
children.push(body("The Evaluator produces scores, confidence, and flags. It does not produce a tier. Section 4 shows the code that does."));

children.push(h2("3.4 Synthesizer (ria/synthesizer.py)"));
...
// ---------- 4. trust gate ----------
children.push(h1("4. The trust gate, in code"));
...
children.push(h2("4.1 Tier decision matrix"));
...
children.push(caption("Table 3. Input provenance. No single component controls more than one input, so one compromised judgment cannot both inflate confidence and launder risk."));

children.push(h2("4.3 Scales and score fields"));
...
children.push(caption("Table 4. Every enum and range a reviewer will meet in outputs and logs, with the component that sets it."));
children.push(new Paragraph({ children:[ new PageBreak() ] }));

// ---------- 5. shared infrastructure ----------
children.push(h1("5. Shared infrastructure"));
children.push(h2("5.1 The cached document context (ria/caching.py)"));
...
children.push(h2("5.2 Retry policy (ria/retry.py)"));
...
children.push(h2("5.3 Cost controls (ria/cost.py)"));
...
// ---------- 6. decisions ----------
children.push(h1("6. Why the decisions were made"));
...
// ---------- 7. data flows ----------
children.push(h1("7. Data flows and security summary"));
...
children.push(bullet([{text:"Outputs: ", bold:true},{text:"local DOCX/PPTX always; a tracker record and escalation email only when the gate decided and a human approved. Logs are local JSONL, SIEM-ready."}]));
...
// ---------- 8. appendix ----------
children.push(h1("8. Appendix: dependencies and repository map"));
children.push(h2("8.1 Pinned dependencies"));
...
children.push(caption("Table 2. requirements.txt, annotated. All versions are pinned for reproducible builds."));
children.push(h2("8.2 Repository map"));
children.push(...codeBlock([
...
children.push(body("Suggested reading order for a technical reviewer: the README's top half for the thesis, ria/evaluator.py for the gate, the agent specs, evaluations/ for the proof, and docs/DESIGN-DECISIONS.md for the reasoning.", { after: 60 }));
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("/home/claude/Riptide-RIA-Technical-Documentation.docx", buf);
