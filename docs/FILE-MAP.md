...
|---|---|
| `C:\Users\poole\Documents\riptide-ria\tests\unit\` | 14 files, offline, no API cost; what CI runs on every push (caching, classifier, evaluator, specialists, synthesizer, Notion client/writer, Gmail client, models, the hooks themselves) |
...
## Addendum: files added in the 2026-07-12 review pass

Kept separate so the snapshot above stays what it says it is. Same conventions apply.

| Path | What it is |
|---|---|
| `C:\Users\poole\Documents\riptide-ria\ria\retry.py` | Shared retryable-vs-fatal API error classification used by classifier/specialists/synthesizer retry loops |
| `C:\Users\poole\Documents\riptide-ria\run_demo.py` | Cross-platform demo runner (macOS/Linux twin of run_demo.bat) |
| `C:\Users\poole\Documents\riptide-ria\evaluations\harness.py` | Pass-rate harness: every cheap live eval runs its case N times and asserts a rate, not one sample |
| `C:\Users\poole\Documents\riptide-ria\evaluations\test_injection_evals.py` | Adversarial suite: injection fixtures vs classifier, specialist, and the Evaluator gate |
| `C:\Users\poole\Documents\riptide-ria\evaluations\results\` | Dated JSON results written by every eval run; committed after green runs as evidence (see its README) |
| `C:\Users\poole\Documents\riptide-ria\tests\unit\test_cost.py` | Cache-aware cost math |
| `C:\Users\poole\Documents\riptide-ria\tests\unit\test_retry.py` | Retryable-vs-fatal classification |
| `C:\Users\poole\Documents\riptide-ria\tests\unit\test_settings.py` | Anthropic-key-only load; Notion enforced at point of use |
| `C:\Users\poole\Documents\riptide-ria\tests\unit\test_federal_register.py` | Markup stripping before the full-text cap |
| `C:\Users\poole\Documents\riptide-ria\docs\ENTERPRISE-FAQ.md` | Vendor-risk Q&A for security/compliance/architecture reviewers (data flows, PHI posture, oversight, audit, integration swaps, cloud paths) |
